<template>
  <div class="app-container" v-loading="isLoading">
    ...
    <el-table :data="data" style="width: 100%" border>
      ...
      <el-table-column label="留言日期" align="center">
...
import { findMessage, delMessage } from "@/api/message";
...
export default {
  ...
  methods: {
    ...
    async fetchData() {
      try {
        this.isLoading = true;
        const res = await findMessage(this.curPage, this.eachPage);
        // 保存数据
    ...
    deleteMessage({ id }) {
      // 确认弹框
      this.$confirm("删除该留言, 是否继续?", "是否删除此留言", {
